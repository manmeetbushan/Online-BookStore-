...
if (isset($_SESSION['UEMAIL'])) {
    ...
    if (isset($_POST['ubook'])) {
        $book_isbn = mysqli_escape_string($connection, $_POST['ubook']);
        $uemail = $_SESSION['UEMAIL'];
        $sql1 = "SELECT CART_QUANTITY FROM cart_data WHERE book_isbn = $book_isbn AND UEMAIL = '$uemail'";
        $result1 = mysqli_query($connection, $sql1);
        $row = mysqli_fetch_assoc($result1);
        $quantity = $row['CART_QUANTITY'] + 1;
        if (mysqli_num_rows($result1) > 0) {
            $sql2 = "UPDATE cart_data SET CART_QUANTITY = $quantity WHERE book_isbn = $book_isbn AND UEMAIL = '$uemail'";
            $result2 = mysqli_query($connection, $sql2);
        } else {
            $sql3 = "INSERT INTO cart_data(book_isbn, UEMAIL, CART_QUANTITY) VALUES ($book_isbn,'$uemail', $quantity)";
            $result3 = mysqli_query($connection, $sql3);
        }
        header('location:ubookpage.php');
        return;
    }
}
?>
...
<body style="overflow-x: hidden;">
                ...
                <ul class="navbar-nav" style="margin-left: auto;">
                    ...
                    <li class="nav-item ">
                        <a class="nav-link active" style="margin-right: 20px;" aria-current="page"
                            href="ubookpage.php">Buy Books</a>
                    </li>
    ...
    if (isset($_SESSION['UEMAIL'])) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <hr><h5 class='card-title text-center'>{$row['book_name']}</h5><hr>
                <img src='../uploads/{$row['book_image']}' class='card-img-top' alt='...'>
                <div class='card-body'>
                    ...
                    <div class='d-grid gap-2'>
                        <button class='btn  shadow-none' type='submit' name='ubook' value={$row['book_isbn']}
                            style='background-color: #adb5db ; border:#adb5db;'>Add to cart</button>
                    </div>
                    </form>
                </div>
            </div>
